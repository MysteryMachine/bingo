...
export const createValues = (sampleSize = 5) =>
  _.flatMap(_.range(0, letters.length), i =>
    selectNums(sampleSize).map(num => ({
      letter: letters[i],
      num: num + i * 15,
      value: letters[i].concat(num + i * 15),
      clicked: false
    }))
  );
...
export const unrandAllValues = _.groupBy(
  _.sortBy(createValues(15), ["num"]),
  o => o.letter
);
...
export const createInitialGameState = () => {
  return {
    ...
    used: [],
    seed: (Math.abs(myrng.int32()) % 100000).toString(),
    winner: false
  };
};
...
export const createBoard = (username, seed) => {
  myrng = seedrandom(username.toLowerCase().concat(seed));
...
export const checkBoard = (gameState, initialGameBoard) => {
...
export const checkWinner = (gameState, username) => {
  ...
  return checkBoard(gameState, initialGameBoard);
};
